...
installed driver supports. `check_flash_attn_ptx_compat` detects this
...
import vllm.ext.platforms.cuda_ext as cuda_ext
...
def test_no_error_on_non_blackwell_consumer_gpu():
    ...
    with (
        patch.object(CudaExt, "_build_cuda_version", return_value=(13, 3)),
        patch.object(CudaExt, "_driver_max_cuda_version", return_value=(13, 0)),
    ):
        CudaExt.check_flash_attn_ptx_compat(DeviceCapability(9, 0))
...
def test_no_error_when_driver_covers_build():
    with (
        patch.object(CudaExt, "_build_cuda_version", return_value=(13, 0)),
        patch.object(CudaExt, "_driver_max_cuda_version", return_value=(13, 3)),
    ):
        CudaExt.check_flash_attn_ptx_compat(DeviceCapability(12, 1))
...
def test_no_error_when_build_version_undetermined():
    ...
    with (
        patch.object(CudaExt, "_build_cuda_version", return_value=None),
        patch.object(CudaExt, "_driver_max_cuda_version", return_value=(13, 0)),
    ):
        CudaExt.check_flash_attn_ptx_compat(DeviceCapability(12, 1))
...
def test_no_error_when_driver_version_undetermined():
    with (
        patch.object(CudaExt, "_build_cuda_version", return_value=(13, 3)),
        patch.object(CudaExt, "_driver_max_cuda_version", return_value=None),
    ):
        CudaExt.check_flash_attn_ptx_compat(DeviceCapability(12, 1))
...
def test_build_cuda_version_parses_local_version_suffix():
    with patch(
        ...
        assert CudaExt._build_cuda_version() == (13, 3)
...
def test_build_cuda_version_falls_back_to_main_cuda_version():
    ...
    with (
        ...
        patch.object(cuda_ext.envs, "VLLM_MAIN_CUDA_VERSION", "13.0"),
    ):
        assert CudaExt._build_cuda_version() == (13, 0)
...
def test_driver_max_cuda_version_handles_nvml_init_failure():
    ...
    with patch.object(
        cuda_ext.pynvml,
        "nvmlInit",
        side_effect=cuda_ext.pynvml.NVMLError(999),
    ):
        assert CudaExt._driver_max_cuda_version() is None


def test_driver_max_cuda_version_shuts_down_nvml_on_query_failure():
    with (
        patch.object(cuda_ext.pynvml, "nvmlInit") as mock_init,
        patch.object(
            cuda_ext.pynvml,
            "nvmlSystemGetCudaDriverVersion_v2",
            side_effect=cuda_ext.pynvml.NVMLError(999),
        ),
        patch.object(cuda_ext.pynvml, "nvmlShutdown") as mock_shutdown,
    ):
        assert CudaExt._driver_max_cuda_version() is None
